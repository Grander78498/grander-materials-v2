...
1. Для каждого узла выбирается признак и порог таким образом, чтобы максимизировать прирост информации. Прирост информации считается на основе индекса Джини (@gini) или энтропии (@entropy):

$ I_G = 1 - sum_(i=1)^J p_i^2 $<gini>
$ "Entropy" = - sum_(i=1)^J (p_i log_2 p_i) $<entropy>
...
Для визуализации исходных данных применён метод главных компонент, суть которого заключается в спектральном разложении ковариационной матрицы. Визуализация исходных данных представлена на Рисунке @data
#figure(image("/img/holm-prac2/holm-prac2_2025-06-05-08-06-37.png"), caption: [Визуализация исходных данных])<data>
...
Результат кластеризации алгоритмом DBSCAN представлен на Рисунке @dbscan-result.
#figure(image("/img/holm-prac2/holm-prac2_2025-06-05-08-09-38.png"), caption: [Результат кластеризации DBSCAN])<dbscan-result>
...
#figure(image("/img/holm-prac2/holm-prac2_2025-06-04-18-47-41.png", height: 25%), caption: [Точность ансамбля в зависимости от количества моделей])<cluster-n-model>

Лучший результат обучения без учителя достигнут при количестве моделей, равном 13.

#figure(image("/img/holm-prac2/holm-prac2_2025-06-04-19-12-20.png", height: 40%), caption: [Результат кластеризации ансамблем])<cluster-ensemble-result>
...
#simple-table(
  ...
  [Дерево решений], [
    ...
    - минимальное количество экземпляров в узле = 2;
    - критерий Джини.
  ], [0.64], [0.62], [00:05], [00:00.01],

  [Бэггинг], [
    - количество моделей = 50;
    - минимальное количество экземпляров в узле = 2;
    - максимальная глубина дерева = 5.
  ], [0.68], [0.66], [02:55], [00:00.05],

  [Случайный лес], [
    - количество моделей = 100;
    - минимальное количество экземпляров в узле = 2;
    - максимальная глубина дерева = 10.
  ], [0.78], [0.76], [00:00.5], [00:00.01],
)
